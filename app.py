...
def calculate_priority(job):
    """Calculates priority with aggressive error handling and smart logging."""
    try:
    ...
    while True:
        try:
            with lock:
                ...
                # PREEMPTION: Pause running AUTO jobs
                ...
                # 3. Assign workers from Dispatch list
                for job in priority_queue[:]:
                    if job.get("suspended"):
                        continue
                    
                    # PRESENTATION MODE: 10s for Bots, 0.5s for Humans
                    ...
                    if not worker and job.get("source") == "WEBHOOK":
                        ...
                        if evict_target:
                            ...
                            evict_target["dispatched_at"] = time.time()
...
<head>
    ...
    <style>
        ...
        .container { display: flex; gap: 20px; justify-content: center; max-width: 1700px; margin: 0 auto; }
        
        .col { flex: 1; background: var(--card-bg); border-radius: 12px; padding: 15px; border: 1px solid var(--border); min-height: 80vh; display: flex; flex-direction: column; }
        .col h2 { font-size: 1.1rem; padding-bottom: 10px; border-bottom: 2px solid var(--border); margin-top: 0; display: flex; justify-content: space-between; }
        .count { background: var(--border); padding: 2px 8px; border-radius: 10px; font-size: 0.8rem; }
        
        .job { background: #21262d; border: 1px solid var(--border); padding: 15px; margin-bottom: 15px; border-radius: 8px; border-left: 5px solid var(--success); transition: transform 0.2s; position: relative; }
        
        .job.suspended { border-left-color: #6e7681; opacity: 0.5; }
        .job.webhook { border-left: 5px solid #ffca28; background: rgba(255, 202, 40, 0.1); box-shadow: 0 0 15px rgba(255, 202, 40, 0.1); border-right: 1px solid #ffca28; }
        
        .job.webhook::after { content: '*'; position: absolute; top: 5px; right: 10px; font-size: 0.6rem; color: #ffca28; font-weight: bold; }
        .suspended-label { font-size: 0.65rem; background: #f85149; color: white; padding: 2px 6px; border-radius: 4px; margin-left: 10px; font-weight: bold; }
        
        .priority-rank { position: absolute; top: 10px; left: -15px; background: var(--accent); color: white; width: 25px; height: 25px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.8rem; font-weight: bold; border: 2px solid var(--bg); }
        
        .job-title { font-weight: 600; margin-bottom: 5px; display: block; }
        .job-meta { font-size: 0.75rem; color: var(--text-dim); text-transform: uppercase; }
        
        .current-stage { color: var(--accent); font-weight: bold; font-size: 0.9rem; margin: 10px 0; display: flex; align-items: center; }
        .current-stage::before { content: '⚡'; margin-right: 5px; animation: pulse 1s infinite; }
        
        @keyframes pulse { 0% { opacity: 1; } 50% { opacity: 0.4; } 100% { opacity: 1; } }
        
        .stages-container { margin-top: 10px; display: flex; flex-direction: column; gap: 4px; }
        .stage-item { font-size: 0.8rem; color: var(--text-dim); display: flex; align-items: center; }
        ...
        .worker-tag { position: absolute; top: 10px; right: 10px; font-size: 0.65rem; background: var(--border); padding: 2px 6px; border-radius: 4px; }
    </style>
</head>
<body>
    ...
    <div class="container">
        ...
        <div class="col" style="background: rgba(88, 166, 255, 0.05);">
            ...
            <div style="margin-left: 20px;">
                ...
                <div class="job {{ 'webhook' if j.source == 'WEBHOOK' else '' }} {{ 'suspended' if j.suspended else '' }}">
                    ...
                    <span class="job-meta">{{ j.language }} {% if j.suspended %}<span class="suspended-label">SUSPENDED</span>{% endif %}</span>
                    <span class="job-title">{{ j.repo }}</span>
        ...
        <div class="col">
            ...
            <div class="job progress-job {{ 'webhook' if j.source == 'WEBHOOK' else '' }} {{ 'suspended' if j.suspended else '' }}">
                ...
                <span class="job-meta">{{ j.language }} {% if j.suspended %}<span class="suspended-label">PREEMPTED</span>{% endif %}</span>
                <span class="job-title">{{ j.repo }}</span>
